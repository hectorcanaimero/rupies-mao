...
class RupiesProfissionalSupabaseUser extends BaseAuthUser {
  RupiesProfissionalSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> rupiesProfissionalSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = RupiesProfissionalSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
